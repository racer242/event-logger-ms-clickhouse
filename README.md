...
> **Примечание:** Тип очереди выбирается через `QUEUE_TYPE`. Доступные значения:
>
> - `sqlite` — очередь на SQLite (по умолчанию, данные сохраняются на диск)
> - `redis` — очередь на Redis (для распределённых систем)
> - `memory` — очередь в памяти (быстро, но данные теряются при рестарте)
